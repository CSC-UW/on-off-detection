...
def run_stdperiod_off_detection(cluster_psth_data, times, params, debug=False):
    ...
    while times[start_idx] <= params["start_tmax"]:
        ...
        end_idx = next(
            idx for idx in range(start_idx + 1, nbins) if datfilt[idx] > thres
        )
        # Fill in period info
...
if __name__ == "__main__":
    ...
    run_stdperiod_off_detection(dat, times, STDPERIOD_PARAMS, debug=True)
